...
export class TechnologyFormComponent implements OnInit {
  ...
  saveTechnology(): void {
    if (this.isEditMode) {
      this.staticDataService.updateTechnology(this.technology); // Update existing technology
    } else {
      this.technology.id = `technology_${Date.now()}`; // Generate a unique ID for a new technology
      this.staticDataService.addTechnology(this.technology); // Add new technology
    }
